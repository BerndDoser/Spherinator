...
class Zernike_embedding(nn.Module):
    ...
    def M_embedding_generator(self, n_max):
        ...
        for m1 in range(0, n_max + 1):
            m1_lengh = lengh - int(
                ((n_max_calc - m1 + 1) * (n_max_calc - m1) / 2) / 2
                + math.ceil((n_max_calc - m1) / 4)
            )
            count = 0
    ...
    def create_filter(self, n_max):
        ...
        for i in range(len(Zernike_functions)):
            ...
            functions.append(
                numpy.polynomial.polynomial.Polynomial(Zernike_functions[i])
            )
        ...
        for i in range(len(Zernike_functions)):
            out.append(
                [
                    functions[i](np.sqrt((x**2 + z**2)))
                    * np.cos(M[i] * np.arctan2(x, (z))),
                    functions[i](np.sqrt((x**2 + z**2)))
                    * np.sin(M[i] * np.arctan2(x, (z))),
                ]
            )
        # print(out[0])
        ...
        norm = torch.tensor(np.array(norm * out_mask), dtype=torch.float, device="cuda")
