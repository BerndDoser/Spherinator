...
def main(
    checkpoint: Path = Path(
        "lightning_logs/version_0/checkpoints/epoch=99-step=9800.ckpt"
    ),
    ...
    scheduler = diffusers.schedulers.DDPMScheduler(
        variance_type="fixed_large", timestep_spacing="trailing"
    )
    ...
    images = torch.stack(
        [tv.transforms.functional.to_tensor(pil_image) for pil_image in pil_images]
    )
